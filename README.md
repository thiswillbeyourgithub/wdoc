...
8. If you want to make sure your data remains private here's an example with ollama: `wdoc --private --llms_api_bases='{"model": "http://localhost:11434", "query_eval_model": "http://localhost:11434"}' --modelname="ollama_chat/gemma:2b" --query_eval_modelname="ollama_chat/gemma:2b" --embed_model="huggingface/BAAI/bge-m3" my_task`. Note that the argument `llms_api_bases` can be used to override the API endpoint per model, meaning that you can override some endpoints and not other, if you want to use ONLY local models you want to use `--private` too. Also, here `--embed_model` points to a huggingface model so will be downloaded and used locally, but you can use ollama by overriding the endpoint in `--llms_api_bases` with `embeddings: "http://localhost:11434"` and `--embed_model="bge-m3"` or even simpler: `--embed_model="ollama/bge-m3"`.
