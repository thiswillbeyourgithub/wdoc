...
### Table of contents
- [What is WinstonDoc in a few questions](#winstondoc-in-a-few-questions)
- [Features](#features)
...
## WinstonDoc in a few questions
* **Who is this for?**
    * WinstonDoc is for power users who want document querying on steroid, and in depth AI powered document summaries.
* **What's RAG?**
...
* **Why is WinstonDoc better than most RAG system to ask questions on documents?**
    * It uses both a strong and query_eval LLM. After finding the appropriate documents using embeddings, the query_eval LLM is used to filter through the documents that don't seem to be about the question, then the strong LLM answers the question based on each remaining documents, then combines them all in a neat markdown. Also WinstonDoc is very customizable.
* **Why can WinstonDoc also produce summaries?**
...
* **What other tasks are supported by WinstonDoc?**
...
* **Which LLM providers are supported by WinstonDoc?**
    * WinstonDoc supports virtually any LLM provider thanks to [litellm](https://docs.litellm.ai/). It even supports local LLM and local embeddings (see [Walkthrough and examples](#Walkthrough-and-examples) section).
* **What do you use WinstonDoc for?**
    * I follow heterogeneous sources to keep up with the news: youtube, website, etc. So thanks to WinstonDoc I can automatically create awesome markdown summaries that end up straight into my [Logseq](https://github.com/logseq/logseq/) database as a bunch of `TODO` blocks.
...
* **Documented** Lots of docstrings, lots of in code comments, detailed `--help` etc. The full usage can be found in the file [USAGE.md](./WinstonDoc/docs/USAGE.md) or via `python -m WinstonDoc --help`.
...
* **Sophisticated faiss saver**: [faiss](https://github.com/facebookresearch/faiss/wiki) is used to quickly find the documents that match an embedding. But instead of storing as a single file, WinstonDoc splits the index into 1 document long index identified by deterministic hashes. When creating a new index, any overlapping document will be automatically reloaded instead of recomputed.
...
* **Static typed**: Runtime type checking. Opt out with an environment flag: `WINSTONDOC_TYPECHECKING="disabled / warn / crash" WinstonDoc` (by default: `warn`). Thanks to [beartype](https://beartype.readthedocs.io/en/latest/) it shouldn't even slow down the code!
* **Scriptable**: You can use WinstonDoc in other python project using `--import_mode`
...
* Improve the scriptability of WinstonDoc. Add examples for how you use it with Logseq.
...
* `pip install -U WinstonDoc`
* Or from the git `dev` branch: `pip install git+https://github.com/thiswillbeyourgithub/WinstonDoc.git@dev`
* Or from the (supposedly more stable) `main` branch: `pip install git+https://github.com/thiswillbeyourgithub/WinstonDoc.git@main`
...
* *Note: if for some reason this fails, maybe try with `python -m WinstonDoc`. And if everything fails, clone this repo and try again after `cd` inside it.*
...
* For shell autocompletion: `eval $(cat completion.cli.zsh)` and `eval $(cat completion.m.zsh)`. You can generate your own with `eval "$(wdoc -- --completion)"` and `eval "$(python -m WinstonDoc -- --completion)"`.
