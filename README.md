...
* This will:
    ...
    5. Pass each of those documents to the smaller LLM (default: gemini/gemini-2.0-flash-001) to tell us if the document seems appropriate given the user query
    ...
    7. Then each relevant doc is sent to the strong LLM (by default, gemini/gemini-2.5-pro-preview-03-25) to extract relevant info and give one answer.
...
* This will:
    ...
    2. pass each chunk into the strong LLM (by default gemini/gemini-2.5-pro-preview-03-25) for a very low level (=with all details) summary. The format is markdown bullet points for each idea and with logical indentation.
