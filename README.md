...
* This will:
    ...
    5. Pass each of those documents to the smaller LLM (default: deepseek/deepseek-v4-flash) to tell us if the document seems appropriate given the user query
    ...
    7. Then each relevant doc is sent to the strong LLM (by default, deepseek/deepseek-v4-pro) to extract relevant info and give one answer per relevant document.
...
* This will:
    ...
    2. pass each chunk into the strong LLM (by default deepseek/deepseek-v4-pro) for a very low level (=with all details) summary. The format is markdown bullet points for each idea and with logical indentation.
