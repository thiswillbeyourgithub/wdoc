...
def load_llm(
    ...
    **extra_model_args,
    ...
    if not private and modelname.startswith("openai/") and api_base is None and no_llm_cache is False:
        red("Using ChatOpenAI instead of litellm because calling openai server anyway and the caching has a bug on langchain side :( The caching works on ChatOpenAI though. More at             More at https://github.com/langchain-ai/langchain/issues/22389")
        llm = ChatOpenAI(
        ...
        red("A bug on langchain's side forces DocToolsLLM to disable the LLM caching. More at             More at https://github.com/langchain-ai/langchain/issues/22389")
        llm = ChatLiteLLM(
